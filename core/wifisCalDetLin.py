...
if(os.path.exists(savename+'_NLCoeff.fits') and os.path.exists(savename+'_satCounts.fits')):
    cont = wifisIO.userInput('Non-linearity processed files already exists for ' +foldername+', do you want to continue processing (y/n)?')
    if (cont.lower() == 'n'):
        contProc = False
    else:
        contProc = True
...
if (contProc):
    ...
    #Read in data
    t1 = time.time()
    data, inttime, hdr = wifisIO.readImgsFromFolder(foldername)
    ...
    data =0 #clean up data cube to reduce memory usage
else:
