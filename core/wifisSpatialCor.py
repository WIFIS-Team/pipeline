    ...
    for j in range(0,nDips):
        ...
        if (dipPos is not np.nan):
            ...
            if (len(xtmp)>2):
                ...
                if (len(xtmp)>2):
                    trace[j],traceAmp[j]  =  getFit2(xtmp, yfit, plot=plot, mxWidth=mxWidth)
        ...
        if (badFit): 
            ...
            if (np.isnan(c)):
                trace[j] = np.nan
            elif (np.abs(c-prevFit) > mxWidth/2.):
                trace[j] = np.nan
            else:
                trace[j] = c
...
def buildAmpMap(traceLst, ampLst, slicesLst):
    """
    """
